# -.- coding:latin1 -.-
# @author: Nicolas
"""Ce code trouve l'énergie par unité de masse d'une exoplanète et la
porte en graphique selon le temps. À noter que u et v sont toujours 
calculés dans une seule et même fonction étant donné que les deux 
formules se ressemblent (et donc que beaucoup de valeurs reviennent 
dans les deux calculs), ce qui permet d'économiser de la puissance de 
calcul en ne faisant qu'une seule fonction. On a suivi le même 
raisonnement pour le calcul de x et de y."""
...
for R in R2:
    # On fait la même démarche pour chaque valeur de R2
    y2[0] = - R
    u2[0] = 2 * np.pi * np.sqrt(M0 / R)
    energie[0] = (u2[0] ** 2 + v2[0] ** 2) / 2 - GM0 / np.sqrt(x1[0] ** 2 + \
y1[0] ** 2) - GM1 / np.sqrt((x2[0] - x1[0]) ** 2 + (y2[0] - y1[0]) ** 2)
    # On définit les conditions initiales qui vont changer avec R2

    for i in range(0, temps.shape[0] - 1):
        x1[i + 1] = x1[i] + h * (u1[i] - h / 2 * (GM0 * x1[i] / ((x1[i] ** 2 + 
y1[i] ** 2) ** (3 / 2))))
        y1[i + 1] = y1[i] + h * (v1[i] - h / 2 * (GM0 / ((x1[i] ** 2 + \
y1[i] ** 2) ** (3 / 2)) * y1[i]))
        u1[i + 1], v1[i + 1] = uv1(x1[i], y1[i], u1[i], v1[i], GM0)
        # On trouve les valeurs de positions en x et en y pour la super-Jupiter

        x2[i + 1], y2[i + 1] = xy(x1[i], x2[i], y1[i], y2[i], u2[i], v2[i], 
GM0, GM1)
...
v1[i], v2[i], GM0, GM1)
        ...
        energie[i] = (u2[i] ** 2 + v2[i] ** 2) / 2 - GM0 / np.sqrt(x2[i] **
2 + y2[i] ** 2) - GM1 / np.sqrt((x2[i] - x1[i]) ** 2 + (y2[i] - y1[i]) ** 2) 

        """On calcule l'énergie totale par unité de masse de la petite 
        exoplanète"""

    plt.plot(temps, energie)
    
    """On montre le graphique de l'évolution de l'énergie totale par unité de
    masse de la petite exoplanète dans le temps"""

plt.xlabel('Temps (en années)')
plt.ylabel('Énergie totale par unité de masse de la petite exoplanète')
plt.ylim(-150, 25)
# On modifie les options esthétiques du graphique

plt.savefig('Laboratoire8-graphiqueQ4.png')
plt.show()